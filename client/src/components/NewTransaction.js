...
export default function NewTransaction(props) {
  ...
  const sendMoney = async (e) => {
    ...
    if (
      ...
      await authAxios
        .post("/createTransaction/send", {
          requester: selectedUser.username,
          requestTo: localStorage.getItem("username"),
          date:
            now.getFullYear() +
            "-" +
          ...
          totalAmountSpent: parseInt(totalAmountSpent),
          budgetRemaining: parseInt(budgetRemaining),
        })
        .then(async (res) => {
          document
            .querySelector(".backgroundNewTransaction")
  ...
  const requestMoney = async (e) => {
    ...
    if (
      ...
      setDisabled(true);
      await authAxios
        .post("/createTransaction/receive", {
          requester: localStorage.getItem("username"),
          requestTo: selectedUser.username,
          date:
            now.getFullYear() +
            "-" +
            ((now.getMonth() + 1 < 10 ? "0" : "") + (now.getMonth() + 1)) +
  ...
  return (
    <div
      className={
        "backgroundNewTransaction w-[100%] h-[100%] fixed top-0 left-0 bg-black/[.548] items-center justify-center inactive"
      }
    >
      <div className="rounded w-[95%] lg:w-[700px] h-[500px] border-[1px] border-gray-300 bg-white shadow-lg relative">
        ...
        >
          <img src={closeIcon} className="w-[20px] rotate-45" alt="close"></img>
        </button>
        <form className="mt-[20%] lg:mt-[10%]">
          <div className="flex flex-row ml-[5%] lg:ml-[15%] mr-[5%] lg:mr-[0%]">
            <p className="text-gray-700 text-[18px] lg:text-[20px] font-semibold lg:mr-10 mb-4 lg:mb-0 w-[170px]">
              Enter amount:{" "}
            </p>
            <p className="text-[25px] font-semibold mr-2">₹</p>
            <input
              type="number"
              className="block h-10 w-[100px] lg:w-[250px] focus:outline-0 hover:border-2 text-[15px] font-semibold hover:border-blue-400 p-2 text-sm bg-gray-200 rounded-md mb-2 lg:mb-0"
              required
              onChange={(e) => {
          ...
          <br />
          <div className="flex flex-row ml-[5%] lg:ml-[15%] mr-[5%] lg:mr-[0%]">
            <p className="text-gray-700 text-[18px] lg:text-[20px] font-semibold lg:mr-10 mb-4 lg:mb-0 w-[190px]">
              Select User:{" "}
            </p>
            <div className="h-[170px]">
              <input
                className="block h-10 w-[150px] lg:w-[250px] focus:outline-0 hover:border-2 hover:border-blue-400 p-2 text-sm cursor-pointer bg-gray-200 rounded-md"
                type="text"
                required
                onChange={(e) => {
                  setSelectedUser(e);
              ...
              <div
                id="usersSelectId"
                className={
                  "h-[120px] max-h-[120px] overflow-y-scroll w-[150px] lg:w-[250px] bg-gray-200 mt-2 pl-2 pt-2 rounded-md flex-col inactive usersSelect"
                }
              >
                {filteredUsers.map((item, key) => {
                  return (
                    <div
                      className="cursor-pointer my-[2px] "
                      key={key}
                      onClick={() => {
                        setSelectedUser({
          ...
          <div className="flex flex-row ml-[5%] lg:ml-[15%] mb-2 mr-[5%] lg:mr-[0%]">
            ...
            <textarea
              ...
              className="text-[15px] font-semibold text-gray-700 block h-[100px] w-64 focus:outline-0 hover:border-2 hover:border-blue-400 p-2 text-sm bg-gray-200 rounded-md mb-2 lg:mb-0"
              onChange={(e) =>
          ...
          <div className="flex flex-row gap-4 justify-center items-center mx-[5%] lg:mx-[0%]">
            <button
              disabled={disabled}
              className={`${
                disabled ? "btn-gray" : "btn-green"
              } w-[150px] lg:w-[180px] h-[40px] lg:h-[40px] mb-2`}
              onClick={(e) => sendMoney(e)}
            >
              <img className="w-5 h-5 mr-2" src={sendIcon} alt="tick"></img>Send
            </button>
            <button
              disabled={disabled}
              className={`${
              ...
              onClick={(e) => requestMoney(e)}
            >
              <img className="w-5 h-5 mr-2" src={receiveIcon} alt="tick"></img>
              Request
            </button>
          </div>
        </form>
